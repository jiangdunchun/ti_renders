...
namespace ti_render {
	sky_object::sky_object(const string& sky_hdr) {
		...
		m_hdr = new gl3plus_texture_2d(sky_hdr_image.get_width(), sky_hdr_image.get_height(), color_format::RGBA16F);
		m_hdr->push_data(sky_hdr_image.get_color_format(), sky_hdr_image.get_data());
	}
