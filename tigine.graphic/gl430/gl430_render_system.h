...
public:
    ...
    GL430CommandQueue  *getCommandQueue() { return command_queue_; }
    GL430CommandBuffer *createCommandBuffer() {
        return new GL430CommandBuffer();
    }
    void release(GL430CommandBuffer *command_buffer) { delete command_buffer; }
    GL430PipelineState *createPipeline(const PipelineStateDescriptor &desc) {
        return new GL430PipelineState(desc);
    }
    void release(GL430PipelineState *pipeline) { delete pipeline; }
